...
function CreatePost() {
      ...
      (error) => {
        switch (error.code) {
          case 'storage/unauthorized':
            ...
            setImageUploadError('User doesn\'t have permission to access the object');
            setImageUploadProgress(null);
            break;
  ...
  const handleCreatePostSubmit = async (req, res) => {
    try {
...
{imageUploadProgress && (
  <Button disabled isProcessing gradientDuoTone="purpleToBlue">Loading....</Button>
)}
       
        </div>
        {data.image && (
          <img src={data.image} alt="uploaded image" width='250' className="m-auto"></img>
        )}
        <ReactQuill
          theme="snow"
